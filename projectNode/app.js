...
const server = http.createServer((req, res) => {
  ...
  if (pathName === '/login') {
    if (method === 'POST') {
      ...
      req.on('end', () => {
        ...
        if (isUserExist) {
          if (usernameForSearch === userAdmin) obj.isAdmin = true;
        }
        // console.log(obj);
  ...
  } else if (pathName === '/templates/usersList') {
    let data;
    try {
      data = fs.readFileSync(__dirname + '/assets/data/users2.json');
    } catch (error) {
      console.log('File not found or wrong file structure');
    }
    let dataList = JSON.parse(data);
    ...
    if (method === 'POST') {
      ...
      req.on('end', () => {
        ...
        dataListAfterAddededObj[0].isSuccess = dataListAfterAddeded !== null;
        res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  } else if (pathName === '/templates/editStudent') {
    ...
    if (method === 'POST') {
      ...
      req.on('end', () => {
        ...
        const parsedBody = Buffer.concat(body); //{"username":"%s","password":"%s"}
        const userDataJson = JSON.parse(parsedBody);
        const originalDataStudent = userDataJson.originalDataStudent;
        const studentName = userDataJson.studentName;
        const studentLastName = userDataJson.studentLastName;
        const studentAge = userDataJson.studentAge;
        const dataListAfterEdit = sFunc.editStudent(
          dataList,
          originalDataStudent,
          studentName,
          studentLastName,
        ...
        //console.log(dataListAfterEdit);
        let dataListAfterEditObj = JSON.parse(dataListAfterEdit);
        dataListAfterEditObj[0].isSuccess = dataListAfterEdit !== null;
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(dataListAfterEditObj));
        //res.end();
      });
      //console.log("RRR" + isUserAuth);
    }
  } else if (pathName === '/templates/deleteStudent') {
    if (method === 'POST') {
      const body = [];
      req.on('data', chunk => {
        body.push(chunk);
      ...
      req.on('end', () => {
        ...
        // let dataList = JSON.parse(data);
        const parsedBody = Buffer.concat(body); //{"username":"%s","password":"%s"}
        const userNameForDelete = JSON.parse(parsedBody);

        // const studentName = userDataJson.studentName;
        // const studentLastName = userDataJson.studentLastName;
        // const studentAge = userDataJson.studentAge;
        const dataListAfterDelete = sFunc.delStudent(userNameForDelete);
        //console.log(dataListAfterDelete);
        // let dataListAfterAddededObj = JSON.parse(dataListAfterAddeded);
        // dataListAfterAddededObj[0].isSuccess = dataListAfterAddeded !== null;
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify(dataListAfterDelete));
        //res.end(dataList);
      });
      //console.log("RRR" + isUserAuth);
    }
